...
| デザインシステム | 色トークン・semantic状態色・タイポ・レイアウト・コンポーネント表現の正本。UI/見た目の実装は本書に一致させる | `DESIGN.md`（repo直下） |
...
- UI・見た目の実装は `DESIGN.md` を正本とし、色・タイポ・レイアウト・状態色を一致させる。デザインを変えるスライスでは、実装と同じ作業内で `DESIGN.md` を更新する。`DESIGN.md` にはデザイン内容だけを書き、開発課題（要件定義・仕様書が持つ）や管理方針（本ファイルが持つ）を混ぜない。
...
- 指示書のスコープ・停止条件を最優先する。
- 正本（docs/・docs/adr/・`DESIGN.md`）を変更したら、AGENTS.md / CLAUDE.md の「詳細仕様の正本」表の一行要約・ポインタと齟齬がないか確認し、必要なら両ファイルを同期更新する。
