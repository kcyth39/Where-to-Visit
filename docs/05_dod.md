...
- [x] オーナー初期セットアップに、確定コピーの3ステップ、お名前、Candidate追加、2種類のURLが順番どおり表示される
- [x] owner URLでの再訪は回答者未選択でも候補一覧を表示し、3ステップを再表示しない。個人名義操作時だけ名前選択へ進む
...
- [x] 候補一覧ダッシュボードにお題・メモとCandidate集約を表示し、回答者別編集controlと判断基準編集を展開していない
