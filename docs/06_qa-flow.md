...
| S24（N6 target・未実装） | canonical relative pathname `^/e/[A-Za-z0-9_-]{43}$`だけを保存し、query／fragment／full URL、raw token別field／派生識別子、外部出力が0であることを確認する。同一pathname upsert、latest 2表示、max 30保持、31件目・expired／malformed／invalid entry purge、180日sliding更新、storage unavailable／SecurityError／quota／JSON parse／write failure時の非阻害、client-only access、SSR localStorage access 0、neutral initial state、hydration mismatch 0、shared browser privacy説明、個別／全削除後のEvent本体存続を確認し、S16のselected participant保存と混同しない |
