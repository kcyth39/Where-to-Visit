...
| S26（N8 canonical four-doc candidate／Human acceptance pending／not execution authorized） | N5〜N7のstacked release lineとcurrent identityを固定し、Production Webのpublic reachabilityを維持してVercel Authenticationを導入・変更しない。Humanがcreator roleを`NOLOGIN`、Data APIをOFFにした後、surface `UNKNOWN 0`とfresh baselineを確認する。nonzero時だけHuman承認済みcleanupとpostcheckを行い、all-zero時は`ALREADY_IN_DESIRED_STATE / CLEANUP MUTATION 0`とする。migration／deploy／Data API再開／role `LOGIN`復帰／Firewall／positive Production smoke／merge／main integrationを実行せずN9へhandoffする |
| S27（N9 target・未実装） | N8 handoff後にmigration、application deployment、必要なData API再開、WAF controlled verification、Production smoke、fixture cleanupを各Human gateで実行・受入し、internal Production acceptanceを完了する。Production access protection方式およびVercel Authentication lifecycleは、N9開始前の別Human decisionで確定する |
...
N8 QAは次の11 scenariosでDoDを観測する。N8の実行順、依存関係、担当、証拠生成順および必要なOperation Packet構成はImplementation Planで定義する。Production Dashboard操作、Production SQL、target guard、artifact identity、Human実行確認、operation count、retry境界および`OUTCOME_UNKNOWN`時のexact手順は、Supabase正本と`operate-supabase-live-db`に従うClass M Operation Packetで定義する。N8ではProduction DB操作を目的とする別Execution Contractを生成しない。Packet adoptionからProduction実行permissionを自動導出しない。

| # | Observation target | Source of truth | Operator | PASS | STOP／OUTCOME_UNKNOWN |
|---:|---|---|---|---|---|
| 1 | release／target identity | Git objects／PR metadata、Supabase／Vercel identity metadata、bounded read-only Web serving-state observation、migration history／catalog identity-only observation | Agent read-only／Human review | exact N5→N6→N7 lineage、current N8 identity、Production target、QA exclusion、Authentication challenge／maintenance interceptionのないpublic reachability、Vercel Authentication／DNS／alias／deploymentのN8 change 0、ownerless migration entry absent。Event作成その他のpositive feature smokeは0 | identity／target mismatchはSTOP。incomplete、lostまたはuncorrelated readはOUTCOME_UNKNOWN |
| 2 | creator route binding | source、deployed binding metadata、role catalog | Agent read-only／Human review | exact `kimenosuke_event_creator`、fallback role／`service_role`／Data API fallback 0 | confirmed mismatchはSTOP。source／deployment／role correlation不成立はOUTCOME_UNKNOWN |
| 3 | NOLOGIN negative lock verification | role／session catalog、authorized operation record、Event／Criterion aggregate | Human mutation／authorized operator／Agent review | `NOLOGIN`、other role delta 0、cleanup-entry active session 0、creation rejected、Event／Criterion delta 0、positive smoke 0、execution bound内、retry 0 | unexpected success、row delta、active sessionまたはbound excessはSTOP。operation／aggregate correlation不成立はOUTCOME_UNKNOWN。追加request 0 |
| 4 | Data API OFF | Dashboard state metadata、REST／GraphQL business-access observation、security fingerprint | Human mutation／Agent review | OFF、REST／GraphQL blocked、schema／RLS／policy／grant delta 0、execution bound内、retry 0 | ON、business access可能またはdriftはSTOP。toggle／denial evidence不完全はOUTCOME_UNKNOWN。再toggle 0 |
| 5 | remaining mutation surfaces | provider metadata、source、catalog、authorized-access inventory | Agent read-only／Human review | Realtime、Storage、Auth、Edge Functions、direct Postgres／pooler、SQL Editorが各`BLOCKED`／`HUMAN_ONLY`／`VERIFIED_N/A`で`UNKNOWN 0` | active unauthorized writerまたはunsafe stateはSTOP。未分類surfaceはOUTCOME_UNKNOWN |
| 6 | counts／dependencies／triggers／schema fingerprint | count-only query、`pg_catalog`、migration history | Human query／Agent review | exact 8 counts、FK／delete actions／external dependencies／triggers、old-owner fingerprint、ownerless migration absentをraw business row 0で確定 | graph／schema mismatch、raw row必要はSTOP。不完全catalog resultはOUTCOME_UNKNOWN |
| 7 | baseline disposition | scenario 6 counts、Human decision identity | Human | any nonzeroは`CLEANUP_REQUIRED`としてscenarios 8〜10へrouteする。all zeroかつHuman acceptanceは`ALREADY_IN_DESIRED_STATE / CLEANUP MUTATION 0`とし、cleanup mutation／artifact／ROLLBACK／COMMIT generation and execution 0 | branchとcountsの不一致またはall-zero未受容はSTOP。decision identity不完全はOUTCOME_UNKNOWN |
| 8 | ROLLBACK | approved operation record、transaction-internal counts、baseline restoration observation | Human execution／Agent review | `CLEANUP_REQUIRED` branchはinternal 8×0、exact baseline restored、SQL error 0、execution 1、retry 0。all-zero branchは`NOT_REQUIRED / EXECUTION 0` | SQL error、baseline mismatchまたはscope deltaはSTOP。restorationまたはbranch correlation不明はOUTCOME_UNKNOWN、COMMIT progression 0 |
| 9 | COMMIT | exact approved operation identity、Human SQL Editor operation record | Human execution／Agent review | `CLEANUP_REQUIRED` branchはexact operation 1、retry 0、known completion。all-zero branchは`NOT_REQUIRED / EXECUTION 0` | known failureはSTOP。completionまたはbranch correlation不明はOUTCOME_UNKNOWN、blind repeat／second execution 0 |
| 10 | postcheck | count-only／catalog／provider metadata、operation ledger | Human query／Agent review | 8×0、dangling／orphan 0、ownerless migration exit absent、schema／security／migration drift 0、creator `NOLOGIN`／exit active session 0、Data API OFF、surface `UNKNOWN 0`、deployment／environment／Firewall／DNS／merge／main integration mutation 0、positive smoke／retry 0 | mismatch／drift／unexpected operationはSTOP。不完全またはuncorrelated evidenceはOUTCOME_UNKNOWN、cleanup rerun 0 |
| 11 | N9 handoff | handoff manifest、evidence identities、Human decision | Tech Lead／Human | release／Production identity、old-owner schema、8×0、role／session、Data API、surface、migration、evidence、unresolved fact 0をHuman accepted | missing／mismatchはSTOP。identity correlation不成立はOUTCOME_UNKNOWN。N9 permission 0 |
