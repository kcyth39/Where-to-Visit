...
| 9 | common SELECT-only exit postcheck | count-only／catalog／provider metadata、operation ledger | Human query／Agent review | nonzero／all-zero両branchで8×0、dangling／orphan 0、ownerless migration exit absent、schema／security／role／grant／migration drift 0、Data API OFF、surface `UNKNOWN 0`、deployment／environment／Firewall／DNS／merge／main integration mutation 0、positive smoke／retry 0。all-zero branchではcleanup mutation／artifact／ROLLBACK／COMMIT 0を維持し、このpostcheckをrace mitigation用のre-count gateとして扱わない | 8×0不成立、mismatch／drift／unexpected rowまたはoperationはSTOP。不完全またはuncorrelated evidenceはOUTCOME_UNKNOWN、cleanup rerun 0 |
| 10 | N9 handoff | handoff manifest、evidence identities、Human decision | Tech Lead／Human | release／Production identity、old-owner schema、8×0、Data API OFF、surface、migration、evidence、unresolved fact 0をHuman accepted | missing／mismatchはSTOP。identity correlation不成立はOUTCOME_UNKNOWN。N9 permission 0 |
