...
| 3 | Data API OFF | Human Supabase Dashboard state、REST／GraphQL business-access classification、security fingerprint | Human mutation／Agent review | exact ProductionでOFF read-back、current REST／GraphQL business access blocked、schema／RLS／policy／grant delta 0、execution bound内、retry 0 | ON、business access可能、wrong targetまたはdriftはSTOP。toggle／denial evidence不完全はOUTCOME_UNKNOWN。再toggle 0 |
...
| 5 | post-lock counts／dependencies／triggers／schema fingerprint | Data API OFF read-back後の最初のcount-only query、`pg_catalog`、migration history | Human query／Agent review | exact 8 counts、FK／delete actions／external dependencies／triggers、old-owner fingerprint、ownerless migration absentをraw business row 0で確定し、この結果をauthoritative baselineとする | graph／schema mismatchまたはbaseline後のbusiness row増加はSTOP。不完全catalog resultはOUTCOME_UNKNOWN |
