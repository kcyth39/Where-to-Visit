...
>
> **N2 Launch Roadmap Rebaseline v4（2026-07-29・N2 CANONICALIZED / CLOSED）:** Human decisionは採用済みで、canonical docsは同期済みである。現行application／DBは旧owner modelのままで、以下のowner URL／owner token／owner Cookie／owner-sessionを検証する項目はhistorical evidenceであって今後のtarget QAではない。N3は`CONTRACT ADOPTED / MODE B / NOT IMPLEMENTATION AUTHORIZED`、N4は`ADOPTED / NOT IMPLEMENTATION AUTHORIZED`、N5は`ENTRY DECISIONS ADOPTED / NOT IMPLEMENTATION AUTHORIZED`、N6〜N13は`PLANNED / NOT IMPLEMENTATION AUTHORIZED`であり、各QAの実行には個別Execution ContractとHuman gateが必要である。
>
> **N5 entry decision QA boundary（2026-07-29・採用済み／未実施）:** [Entry Decision Contract](contracts/WTV-N5-ENTRY-DECISION-CONTRACT-v0.1-draft.md)のD1〜D7とerror copyはHuman採用済みだが、actual QA project／role／password／credentialは未作成、dependency／environment bindingは未追加・未設定、Layer 2 replay／Preview／credential／connection QAは未実施である。actual resource identityは後続resource creation recordでfresh固定する。N3がpackage／lockfileを所有中の場合はN5が同pathを変更せずSTOPする。gate名、Contract保持または本記録からresource作成、dependency追加、実装、QA実行のpermissionを導出しない。
>
