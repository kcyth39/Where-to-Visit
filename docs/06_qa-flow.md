...
| S24（N6 target・未実装） | canonical relative pathname `^/e/[A-Za-z0-9_-]{43}$`、表示用title、`lastVisitedAt`、`expiresAt`だけを保存し、titleを唯一のEvent由来情報の限定例外として扱う。title以外のEvent business data、query／fragment／full URL、raw token別field／派生識別子、外部出力が0であることを確認する。同一pathname upsert、latest 2表示、max 30保持、31件目・expired／malformed／invalid entry purge、180日sliding更新を確認する。read failure（`getItem`例外・storage unavailable）、write failure（quota・`setItem`例外）、remove failure（`removeItem`例外）を個別に検証し、各caseでEvent作成・share page表示・共同編集を阻害せず、history UIをneutral／disabled／emptyへfallbackし、N6履歴key以外の変更、selected participant keyの変更、Event本体DB mutation、localStorage全体clearを0とする。client-only access、SSR localStorage access 0、neutral initial state、hydration mismatch 0、shared browser privacy説明、個別／全削除後のEvent本体存続を確認し、S16のselected participant保存と混同しない |
| S25（N7 policy rebaseline candidate・未実装） | exact `POST /api/events`向けprovisional `60 requests / 600 seconds / IP / fixed window / HTTP 429`をregion-scoped operational parameterとして扱い、global exact quotaまたは個人単位の利用権を主張しない。後続Planはprovider semantics、mutation count、fixture数、DB cost、cleanup安全性を踏まえ、configured threshold-minus-one／threshold／threshold-plus-oneのbounded testか、別Human承認済みtemporary lower QA thresholdのどちらかを一意に選ぶ。Contract段階では60件の恒久Event作成またはtemporary mutationを自動要求しない。controlled QAではexact route／method、429のbody parse前分類、draft保持、navigation／retry／N6 history mutation 0、rejected Event／Criterion row delta 0、accepted atomicity、window behavior、他route・Production非干渉、cleanup／retentionを確認する。shared-IPはsimulation／Human reviewでfalse positiveを評価し、raw IPを証拠化しない。region-scoped結果をglobal exact保証へ拡張しない |
| S26（N8 Production maintenance target／not execution authorized） | N5〜N7のstacked release line、current N8 identityおよびold-owner／Data API-based Production stateを固定し、Production Webのpublic reachabilityを維持してVercel Authenticationを導入・変更しない。HumanがData APIをOFFにしてcurrent REST／GraphQL business accessを停止した後、remaining surface `UNKNOWN 0`と最初のfresh baselineを確認する。nonzero時だけHuman承認済みcleanupとpostcheckを行い、all-zero時は`ALREADY_IN_DESIRED_STATE / CLEANUP MUTATION 0`とする。creator role mutation、migration／deploy／Data API再開／Firewall／positive Production smoke／merge／main integrationを実行せずN9へhandoffする |
| S27（N9 ownerless Production release target／not execution authorized） | current N9 requirementsと§2.5に従い、N8 CLOSED stateからexact ownerless migration、environment-separated credential binding、cumulative deployment、Vercel Authentication isolation、Production Firewall、Data API ON、minimal internal smoke、correlated fixture cleanupおよびcloseoutを別Human gateで検証する。QA evidenceをProduction proofへ代用せず、`N9_COMPLETE`／`N9_STOP`／`N9_OUTCOME_UNKNOWN`へ分類する |
...
| S29（N11-a target・未実装） | N9開始前の別Human decisionで確定したProduction access protection下で利用規約、実際の広告無効状態と一致するPrivacy、affiliate／PR guideline、問い合わせ導線、support受信可能性を確認し、一般アクセスを開始しない |
...
| S31（N11-c target・未実装） | public pagesのrobots／canonical／sitemap、Event pagesの`noindex`とtest、Search Console ownership準備のrunbook／対象値、domain／SSL、backup／recovery、browser／mobile／accessibility、launch checklistをN9開始前の別Human decisionで確定したProduction access protection下で確認する。sitemap送信とindex requestは行わない |
| S32（N12 target・未実装） | N9開始前の別Human decisionで確定したProduction access protection方式とVercel Authentication lifecycleに従って最終受入を行い、ownerless Data API、Event作成WAF blockのcurrentness、public pagesのindex設定、Event pagesの`noindex`を順に確認する。一般公開に必要な別Human gateの後に外部相当browserで閲覧／作成／rate limitを確認し、公開後の実traffic観測へhandoffする。sitemap送信、index requestはHuman操作またはwaiverを記録してlaunchを宣言する |
...
「きめごと／きめごと一覧」は権限ではない同一ブラウザ向けの戻り道としてN6で実装・検証し、S16のselected participant回帰QAと混同しない。N9はinternal Production acceptance、N12はpublic-opening gate、N13は一般公開後の広告activationとして分離する。N9のProduction access protection方式およびVercel Authentication lifecycleは、N9開始前の別Human decisionで確定し、N8の完了条件、handoff stateまたはN8から導出される既定状態として扱わない。

N12の順序は、N9開始前の別Human decisionで確定したProduction access protection方式とVercel Authentication lifecycleに従う最終Production受入、ownerless Data API確認、Event作成WAF block確認、public pagesの`noindex`解除、Event pagesの`noindex`再確認、一般公開に必要な別Human gate、外部相当browserでの閲覧・作成・rate limit確認、sitemap送信またはwaiver、index requestまたはwaiver、launch declarationとする。N13はこの順序のblockerにしない。
...
### 2.4 N8 Production maintenance QA contract

N8 QAは次の10 scenariosでDoDを観測する。N8の実行順、依存関係、担当、証拠生成順および必要なOperation Packet構成は、本canonical定義に基づくcurrent Human-adopted Implementation Planが定義する。current adopted Planが存在しない間はN8を実行しない。Production Dashboard操作、Production SQL、target guard、artifact identity、Human実行確認、operation count、retry境界および`OUTCOME_UNKNOWN`時のexact手順は、Supabase正本と`operate-supabase-live-db`に従うcurrent Human-adopted Class M Operation Packetが定義する。current adopted Packetが存在しない間は対応するProduction操作を実行しない。pre-rebase Plan／Packetはhistorical frozen artifactであり、current execution authorityとして使用しない。N8ではProduction DB操作を目的とする別Execution Contractを生成しない。Plan／PacketのadoptionからProduction execution permissionを自動導出しない。

**Final observation（2026-08-05）:** 10 scenariosはHuman closeout acceptanceにより完了し、`N8 CLOSED / N9 NOT STARTED・NOT AUTHORIZED`である。Data API provider mutationは1回、successful SQL Editor executionはdiscovery 1、ROLLBACK 1、COMMIT 1、postcheck 1、retryは全件0である。cleanup前のexact 8-table totalは36、cleanup／postcheck後は全8 tablesとdangling／orphan rowが0、required old-owner markersはpresent、ownerless target artifactはabsent、expected old-owner structure digestは`OLD_OWNER_MISMATCH`である。Human decision `N8_OLD_OWNER_MISMATCH_REBASELINE_ACCEPTED`はmismatchを消去・PASS変換せず、N9 entry baselineとして受容した。先行fingerprint query失敗3件はmutation 0／authority 0のhistorical diagnosticsで、final authorized chainのPASS根拠へ使用しない。

Final evidenceは`/Users/shige/Projects/Where-to-Visit-Evidence/N8-production-maintenance/20260805T124529Z-n8-final-closeout/`（`COMPLETE` SHA-256 `61835b3b13237e1fc1d32f4e11d4b1568fc55fa86166b24805cda17aac179965`）、additive disposition evidenceは`/Users/shige/Projects/Where-to-Visit-Evidence/N8-production-maintenance/20260805T130247Z-n8-old-owner-mismatch-disposition/`（`COMPLETE` SHA-256 `4ab63a3ed41a6df7080c43cb90545e9dde23498f9c604dbf6c1d51d8364c00f7`）へno-replaceで保持する。Phase 2 raw resultはbyte-identicalに保持した。ROLLBACK／COMMIT／postcheckの独立raw outputは保持されていなかったため再構築せず、Human受容済み結果をsummary／operation ledgerとして明示した。このevidence-retention limitationはaccepted resultを変更せず、N9 permissionも生成しない。

| # | Observation target | Source of truth | Operator | PASS | STOP／OUTCOME_UNKNOWN |
|---:|---|---|---|---|---|
| 1 | release／Production target／current application identity | Git objects／PR metadata、repository source、Vercel deployment metadata、Human Supabase Dashboard、later-authorized SQL catalog observation | Agent read-only／Human review | exact N5→N6→N7 lineage、current N8 identity、old-owner／Data API-based Production state、exact Production target、QA exclusion、ownerless migration entry absent。Vercelはdeployment／environment／source／serving state、Dashboardはproject ref／Data API state、SQL Editorはdatabase／schema／catalog targetをそれぞれ証明し、単独sourceから全targetを推定しない | confirmed identity／target／current-path mismatchはSTOP。必要sourceがincomplete、lostまたはuncorrelatedならOUTCOME_UNKNOWN。Vercel interfaceがSupabase refを値非表示で返せないこと単独ではSTOPにしない |
| 2 | public serving state | Vercel deployment／alias metadata、bounded public-origin observation | Agent read-only／Human review | public originが到達可能で、Authentication challenge／maintenance interceptionがなく、Vercel Authentication／DNS／alias／deploymentのN8 change 0。最初の応答が301／302／307／308でも、approved same-site canonical Production originへのredirect、loop 0、外部／未承認target 0ならPASS可能。body captureは不要 | external／unapproved redirect、redirect loop、Authentication challenge、maintenance interceptionまたはconfirmed public-origin failureはSTOP。target／loop／serving classification不完全はOUTCOME_UNKNOWN |
| 3 | Data API OFF | Human Supabase Dashboard state、REST／GraphQL business-access classification | Human mutation／Agent review | exact ProductionでOFF read-back、current REST／GraphQL business access blocked、execution bound内、retry 0、second toggle 0 | ON、business access可能、wrong targetまたはbound違反はSTOP。toggle／denial evidence不完全はOUTCOME_UNKNOWN。再toggle 0 |
| 4 | remaining mutation surfaces | provider metadata、source、catalog、authorized-access inventory | Agent read-only／Human review | Realtime、Storage、Auth、Edge Functions、direct Postgres／pooler、SQL Editorが各`BLOCKED`／`HUMAN_ONLY`／`VERIFIED_N/A`で`UNKNOWN 0`。Data API OFFから他surfaceの停止を推定しない | active unauthorized writerまたはunsafe stateはSTOP。未分類surfaceはOUTCOME_UNKNOWN |
| 5 | post-lock counts／dependencies／triggers／schema fingerprint | Data API OFF read-backおよびsurface acceptance後の最初のcount-only query、`pg_catalog`、migration history | Human query／Agent review | schema structure、RLS、policies、grants、FK／delete actions／external dependencies／triggers、old-owner fingerprint、ownerless migration absenceおよびexact 8-table fresh countsをraw business row 0で確定し、この最初のcomplete resultをauthoritative catalog／count baselineとする | graph／schema／security mismatchまたはbaseline後のbusiness row増加はSTOP。不完全catalog resultはOUTCOME_UNKNOWN |
| 6 | baseline disposition | scenario 5 counts、Human decision identity | Human | any nonzeroは`CLEANUP_REQUIRED`としてscenarios 7〜9へrouteする。all zeroかつHuman acceptanceは`ALREADY_IN_DESIRED_STATE / CLEANUP MUTATION 0`とし、scenarios 7／8を`NOT_REQUIRED / EXECUTION 0`としてscenario 9のSELECT-only exit postcheckへ進む。cleanup mutation／artifact／ROLLBACK／COMMIT generation and executionは0 | branchとcountsの不一致またはall-zero未受容はSTOP。decision identity不完全はOUTCOME_UNKNOWN |
| 7 | ROLLBACK | approved operation record、transaction-internal counts、baseline restoration observation | Human execution／Agent review | `CLEANUP_REQUIRED` branchはinternal 8×0、exact baseline restored、SQL error 0、execution 1、retry 0。all-zero branchは`NOT_REQUIRED / EXECUTION 0` | SQL error、baseline mismatchまたはscope deltaはSTOP。restorationまたはbranch correlation不明はOUTCOME_UNKNOWN、COMMIT progression 0 |
| 8 | COMMIT | exact approved operation identity、Human SQL Editor operation record | Human execution／Agent review | `CLEANUP_REQUIRED` branchはexact operation 1、retry 0、known completion。all-zero branchは`NOT_REQUIRED / EXECUTION 0` | known failureはSTOP。completionまたはbranch correlation不明はOUTCOME_UNKNOWN、blind repeat／second execution 0 |
| 9 | common SELECT-only exit postcheck | count-only／catalog／provider metadata、operation ledger | Human query／Agent review | nonzero／all-zero両branchで8×0、dangling／orphan 0、ownerless migration exit absent、N8 executionによるschema／security／role／grant／migration delta 0、Data API OFF、surface `UNKNOWN 0`、deployment／environment／Firewall／DNS／merge／main integration mutation 0、positive smoke／retry 0。受容済みentry baselineの`OLD_OWNER_MISMATCH`をdelta 0へ読み替えない。all-zero branchではcleanup mutation／artifact／ROLLBACK／COMMIT 0を維持し、このpostcheckをrace mitigation用のre-count gateとして扱わない | 8×0不成立、未受容のmismatch／drift／unexpected rowまたはoperationはSTOP。不完全またはuncorrelated evidenceはOUTCOME_UNKNOWN、cleanup rerun 0 |
| 10 | N9 handoff | handoff manifest、evidence identities、Human decision | Tech Lead／Human | release／Production identity、pre-ownerless／old-owner family、required old-owner markers present、`OLD_OWNER_MISMATCH`、ownerless target absent、8×0、Data API OFF、surface、migration、evidenceをHuman accepted | missingまたは未記録のmismatchはSTOP。identity correlation不成立はOUTCOME_UNKNOWN。N9 permission 0 |

N8は、Data API OFF時点にin-flight requestが存在しないというHuman-adopted前提を用いる。quiet period、request／connection-pool drain、wait timer、in-flight count、race-specific table lock、retryまたはre-count gateを追加しない。scenario 5の最初のfresh countをauthoritative baselineとし、その後のrow増加をunexpected mutationとしてSTOPする。public serving stateはN8のnon-mutationを支える観測であり、maintenance lockではない。

すべてのlive、providerまたはDB observationはexact 1つの`PASS`、`STOP`、`OUTCOME_UNKNOWN`へ分類する。`OUTCOME_UNKNOWN`はPASSではなく、automatic retry、blind repeatおよびnext-stage progressionを0とする。local parser、formattingまたはsanitizer errorは、追加live request 0、authority／target／evidence meaning不変、secret protection維持の範囲だけで自律修正できる。

### 2.5 N9 ownerless Production release QA contract

本節は[DoD §3.8](05_dod.md#38-n9-ownerless-production-releasenot-execution-authorized)のsystem stateをどう観測するかを定義する。本節のdrafting／review／adoptionからContract、Plan、Operation Packetのdrafting／adoption authorityまたはoperation／execution permissionを生成しない。実行順、exact SQL／provider操作、credential取扱いおよびmutation authorityは後続の別Human-adopted Contract／Plan／bounded operation authorityが定義し、それらのadoptionからもexecution permissionを自動導出しない。

#### Current lifecycle and evidence

- Stage 0は`N9_STAGE_0_COMPLETE_ZERO_DELTA`としてHuman accepted、Stage 1は`N9_STAGE_1_COMPLETE_ZERO_SOURCE_DELTA`としてtechnical completeである。GraphQLは`OUT OF SCOPE / NOT REQUIRED`、Stage 2は`NOT STARTED / NOT AUTHORIZED`、Production N9 executionは`NOT STARTED / NOT AUTHORIZED`である。
- Stage 1 evidenceは、Localのisolated runtime／creator route／`/api/events`／M01–M11／catalog／RLS／pgTAP 118／N6 history／N7 429／regression／check／build、QAのexact target／profile／catalog／RLS／Preview→QA／positive／negative／REST／Data API／creator path／fixture cleanup／final 8×0を含む。GraphQL evidenceは要求しない。
- `N9_SOURCE_DELTA_ZERO_CONFIRMED`は、N9 product behavior、Production ownerless migration bodyおよびN9 architectureの新規deltaが0であることを示す。Stage 1を成立させたtracked tooling／runtime-routing／test／operational documentation deltaの不存在を意味しない。Stage 2でfresh inventoryし、commit対象を分類する。

#### Environment separation

N9のrequired data-access surfaceはSupabase REST／Data APIとserver-side direct PostgreSQL creator routeである。GraphQLはproduct／data-access architectureとして採用せず、`OUT OF SCOPE / NOT REQUIRED`とする。

- **Local:** exact target／ownershipが確認された再構築可能環境では、Agentがruntime、DB reset、migration replay、SQL、schema／role／grant／policy、fixture、destructive test、Local credential provisioning、cleanupおよびbounded retryを実行できる。Local／Remote分離、複数stack ownership分離、localhost-only、target-owned cleanup、Remote credential不使用、secret非表示を守る。
- **Supabase QA:** Humanが承認したbounded QA validation scope内では、AgentがDB read／write、approved migration validation／適用、task-correlated fixture、REST／Data API／RLS／permission、creator path、bounded retryおよびcleanupを実行できる。通常のQA read／mutation／fixture／cleanupを個別micro-gateへ分けない。target ambiguity、Production separation不能、ownership不明data、QA全体reset、共有QAへの広範囲影響、credential発行／rotation、provider／environment binding変更、Data API／Auth／network設定変更またはProduction operationはHumanへ戻す。
- **Production:** Authentication、migration、credential provision、Vercel binding、deployment、Firewall、Data API ON、minimal internal smoke、fixture cleanupおよびfinal postcheckを、それぞれ別Human gate成立後だけ実行する。QA／Preview evidenceをProduction proofへ代用しない。

#### Compact QA matrix

| Requirement area | Local | QA | Production | Evidence | STOP |
|---|---|---|---|---|---|
| Identity／target | repository Head、migration／SQL artifact SHAを固定 | QA ref、Preview environment、QA bindingを相関 | Production ref、Vercel project／environment／deployment／alias、exact release Headを相関 | Git object、artifact identity、provider metadata、secret-free target classification | Head／SHA／ref／environment mismatch、QA／Production crossover、uncorrelated evidence |
| QA security catalog | M11 clean-chainとrole／grant／policy negative fixtures | bounded SELECT-onlyでcreator attributes／membership／ownership、database direct grantはCONNECTだけ、direct TEMP grant／database CREATE 0、PUBLIC／provider default由来effective TEMPの分離、password PRESENT／ABSENT分類、Event policies、exact 8 tablesのauthenticated-applicable policy 0、table／column grants、other 7 denial、function EXECUTE、RLS、old-owner absent、ownerless present、dependency、8 countsをfresh確認。effective TEMPはapplication authorizationへ昇格せず、applicationのarbitrary SQL／temporary object作成を0とする | Production migration前conditionの別readへ引き継ぐだけ | exact one target-correlated catalog projection、direct／PUBLIC ACL分類、raw password／verifier／function body／business-row values 0 | catalog unknown、unexpected direct TEMP grant、database CREATE、creator privilege過大、required denial／marker／RLS failure。PUBLIC／provider default由来effective TEMPだけをmigration gapと扱わない |
| Migration | exact M11 bodyのclean-chain、atomicity、zero-row fixture、postcheck | current M11 stateを確認し再適用0 | exact approved same body、precondition PASS、Human execution 1、post-migration catalog／8×0、retry 0 | body SHA、pre／post catalog projection、Human operation record | body／precondition mismatch、row nonzero、partial state、retry requirement、unknown outcome |
| Credential／binding | fail-closed config、server-only import boundary、client bundle exposure 0、service-role import／fallback／use 0、TLS behavior | Preview scope→QA、server-only、CA／TLS、service-role absent | Production scope→Production、post-deploy再相関、password classificationだけのdelta | source／bundle static scan、value-free presence／scope／target／role classification、provider metadata | reuse、scope／target mismatch、service-role binding／import／fallback、client exposure、privilege／membership delta |
| Deployment／Authentication／Firewall | creator route、N6 history、N7 429のfocused regression | Preview deployment／bindingとapplicable WAF evidence | Authentication current state、OFF時だけ別Human enable、retry 0、ON read-back／external denial。exact cumulative Head `READY`、activation直前fresh 8×0、alias、route inventory。Firewall active／draft／versions inventory、unknown draft 0、exact diff／activation version、draft mutationとactivationの分離、active read-back、unrelated delta 0 | deployment／alias／environment identity、Authentication current／mutation count／ON read-back identity、fresh aggregate、Firewall draft／version／active identity、controlled observation | Authentication outcome不明／ineffective、Head／alias／binding mismatch、fresh row nonzero／target uncorrelated、unknown draft、Firewall activation outcome不明／inactive、Preview substitution。blind retry 0 |
| Data API | REST／Data API contractとdirect INSERT negative fixture | ON stateでREST／Data APIを評価し、collaborationとexact 8 tablesへのbounded authenticated business access 0を確認する。GraphQLは`OUT OF SCOPE / NOT REQUIRED`とする | post-migration security、deployment／binding／isolation、pre-ON creator-route negative／readiness、Firewall active／controlled PASS後だけHuman toggle 1、ON read-back、REST／Data API確認、direct Event INSERT denial、authenticated business access 0 | Dashboard state、prerequisite identities、surface別status／bounded result、operation count | prerequisite不成立、ambiguous toggle、surface不一致、direct INSERT成功、unexpected authenticated access |
| HTTP response matrix | malformed `400/failed`、shape `503/failed`、invalid `400/invalid`、success `201/created`、known `503/failed`、unknown `503/outcome_unknown`、429をroute mock／DB fixtureで区別 | exact cumulative Headのcreator routeでmatrix、Criterion failure時のEvent／Criterion／Participant delta 0、Data API behaviorを確認 | fault injection 0。minimal `201`と別controlled `429`だけを確認 | status／category、request count、DB dispatch／row delta、draft／navigation／retry／historyのboolean assertions | response誤分類、unexpected dispatch／row、retry／navigation／history mutation |
| Functional smoke | Participant／Candidate／Criterion／Vote／Reaction／Concern／Comment、token／cross-Event／title／permission regression | required operation family、positive／negative paths、N6 successful share lookup後だけのhistory更新、failed／unknown history 0、fixture cleanup | exact correlated Event 1件、default Criterion、share pageとsuccessful lookup history、one collaboration path、REST／Data API negative 1、direct Event INSERT denial、creator negatives、unexpected row 0 | fixture correlation、history mutation有無を含むboolean behavior、exact 8 aggregate | extra fixture／row、atomicity／denial／share／history failure、correlation incomplete |
| Abuse mitigation | 429 body non-consumption、canonical copy、draft保持、navigation／retry／history mutation 0をmock検証 | 同じboolean assertionsを確認し、Preview evidenceはProduction proofと分離 | exact POST rule／Production scope／current Human-approved provider-semantic parameter、activation後active read-back、controlled 429、body read 0、canonical copy、draft保持、navigation／retry／history 0、rejected row 0。PATCH／draft mutation成功だけではPASSしない | configuration／draft／activation version identity、status category、boolean-only UI assertions、aggregate row delta | global exact claim、rule inactive／activation outcome unknown、unrelated delta、429 row delta／body use／client state mutation。blind retry 0 |
| Cleanup／closeout | cleanup artifact guardをfixtureで検証 | QA fixture 0へ戻す | exact correlated fixtureだけをHuman cleanup、COMMIT／postcheck、8×0、Data API ON、Authentication ON、ownerless state、unexpected drift 0 | artifact／fixture identity、operation count、postcheck、Human acceptance。既知pre-existing／provider-managed ACL varianceは別分類とeffective denialで扱う | non-correlated target、persisted row、unexpected drift、missing evidence、unresolved outcome |

#### Evidence, retry and terminal policy

- Evidenceはsecret-free、target-correlated、environment-specific、operation-count／retry-awareとし、raw business row、raw share pathname、raw function body、DB URL、password／verifier、token、credential value、CA本文、Cookieまたはsession identifierを取得・保存しない。必要十分なbounded projectionを用い、generic evidence frameworkを作らない。
- GitHub、VercelおよびSupabaseはConnector-firstとし、CLI loginをconnector fallbackとして開始しない。
- Production mutation retryは0とする。SQL error、target ambiguity、result truncationまたはoutcome uncertainty後のread-only repeatも自動承認しない。QA environment mutation retryは後続Contract／Planで別途固定する。
- target mismatch、cross-environment binding、service-role application binding、required QA catalog unknown、migration precondition mismatch、migration前Production row nonzero、creator privilege過大、direct Event INSERT成功、unexpected authenticated business access、ambiguous Data API toggle、deployment Head mismatch、Authentication ineffective、Firewall inactive、controlled 429 row delta、unexpected Production row、cleanup correlation incomplete、unauthorized retry、result truncation／missing evidenceまたはunresolved `OUTCOME_UNKNOWN`ではSTOPする。
- Local defectはProduction／provider request 0、authority／target／meaning不変の範囲で修正・再実行できる。live operationのknown failureをlocal defectとして扱わない。
- 全DoDが同一release stateで成立した場合だけtechnical QA classificationを`N9_COMPLETE`、known mismatch／denial failure／unauthorized operationを`N9_STOP`、結果または相関を一意化できない場合を`N9_OUTCOME_UNKNOWN`とする。partial successをcompleteへ昇格しない。`N9_COMPLETE`からHuman N9 closeout acceptance、public launch、N12、N13、mergeその他のpermissionを生成しない。

Production smokeはzero-row／no-real-user entryを利用した上記matrixの最小scopeに限定し、full regression、historical ACL provenance、broad Production read、one Packet per test、重複REST／Data API setupまたはmigration compatibility layerを要求しない。REST／Data APIは同じcorrelated fixtureを共有できる。GraphQLはN9のrequired surfaceではない。

Current lifecycleは`STAGE 1 TECHNICALLY COMPLETE / CANONICAL RE-ADOPTION PENDING`である。Human re-adoption orderはRequirements → DoD／QA → Execution Contract → Execution Planとし、4層のre-adoption後の次Human decisionは`N9_STAGE_2_GIT_INTEGRATION_AUTHORIZATION`である。本節のre-adoptionからStage 2、Git publication、Production operationまたはcloseout permissionを自動生成しない。
