...
>
> **N5 task-branch QA boundary（2026-07-30・implementation candidate／not main-integrated）:** [Entry Decision Contract](contracts/WTV-N5-ENTRY-DECISION-CONTRACT-v0.1-draft.md)のD1〜D7とerror copyはHuman採用済みで、implementation startとN5専用dependency installは別Human gateで承認済みである。有効なmain baseline `87295a19f80192ffbe91c56dded86748d3a51bbd`は旧owner model、branch `codex/n5-ownerless-transition`のownerless変更は実装候補であり、main実装済みまたは受入済みとは扱わない。QA resource `where-to-visit-qa`（ref `twcbycyyrxbovtgiqaun`）の作成は`COMPLETE_BY_HUMAN`、creation record reviewは`APPROVED_BY_HUMAN`（SHA-256 `cca7c110c7152574c689ac10d01a4e4c85e105d7f3331755982bfbc741569f76`）である。approved external recordをmain統合までresource identity authorityとし、canonical docs synchronizationは`PENDING`を維持する。role／password／credential／environment bindingは未設定、Layer 2 replay／Preview／connection QAとsame-SHA `H5` acceptanceは未実施である。exact `C5`はGit publication／evidenceで別途固定する。resource作成またはrecord承認からcredential、replay、binding、QA、Git、merge、Productionのpermissionを導出しない。
>
...
| S23（N5 task-branch candidate／Layer 2・H5 pending） | ownerless Event作成、共有URL一本化、title不変、memo共同編集、owner route／token／Cookie／session／認可fallback不在、Participant等の既存共同編集回帰をUI／server／DBで確認する。memoはLF normalization、ECMAScript trim、Unicode scalar value count最大1000、unpaired surrogate拒否、DB `char_length` parityとexact error copyを同一verdictにする。採用済みdriver／connection contract、least-privilege role、Layer 2 replay、Preview binding、credential lifecycleは各別Human gate後に確認し、approved QA resource identityとN3 package／lockfile ownershipをpreflightする。task-branch test結果をmain受入またはLayer 2／H5の代替にしない |
...
- QA project resourceとcreation recordの完了はresource identityだけを確定し、migration replay、role credential、Preview binding／QAを実施済みまたは許可済みと扱わない。project refまたはapproved record SHAがdriftした場合は停止してHuman reviewへ戻る。
- task-branch candidateのDB-independent QA、local DB QA、local credential QA、Layer 2 proofを別判定にし、`SKIP`／`NOT RUN`を`PASS`へ読み替えない。
- exact `C5`はGit／immutable evidenceで固定する。Layer 2は別Human gate後にfresh targetで実行し、first failureではtargetをtaintedとして停止する。C5変更時は同じproofを継承しない。
- Layer 2 replay、least-privilege role／GRANT／RLS、Preview binding、exact one smoke、cleanup、business row 0、C5 drift 0を完了するまでC5を`H5`へpromoteしない。`H5`はC5から変更0のsame SHAとし、N6 handoffは別Human gateにする。
