...
| [`adr/0002`](adr/0002-tech-stack.md)、[`0003`](adr/0003-evaluation-and-decision-logic.md)、[`0004`](adr/0004-permission-model.md)、[`0005`](adr/0005-drop-attribute-dynamic-criteria.md)、[`0006`](adr/0006-collaborative-response-row-model.md)、[`0007`](adr/0007-event-views-and-criterion-feedback.md)、[`0008`](adr/0008-local-supabase-development-workflow.md)、[`0009`](adr/0009-ownerless-collaborative-model.md) | architecture・data・permission・運用上の決定と理由。ADR-0009はownerless collaborative modelを採用済みだが未実装 | 技術／product決定正本・`CURRENT CANONICAL` | Tech Lead（技術）／Claude（product意味）／Human（重要risk） | PKA | 03〜06に結論が反映される | 後続ADRと03〜06の明示優先関係に従う。owner固有仕様はADR-0009を優先 | 03〜06、Skill | AGENTS／CLAUDE、実装contract | architecture・product意味・安全境界の承認済み変更 |
...
| [`contracts/`](contracts/README.md) | Human採用済みtask-specific Execution Contractのreviewed body、review／adoption identity、current lifecycleの保持とrouting | Contract保持索引・`CURRENT CANONICAL`。採用済みContractは`APPROVED / NOT IMPLEMENTED`であり、保存だけでは実装permissionを生成しない | 各Contractのdomain owner／Human（採用・risk） | PKA | Roadmap、03〜06、chat上の採用記録とidentityが重なる | body内statusはartifact生成時snapshot。current lifecycleはexact body／adoption SHAを持つ索引を優先し、execution evidenceを混在させない | Roadmap、03〜06、Git外evidence | AGENTS／CLAUDE、各Execution Contract | Contract採用・置換、review／adoption identity、expiry、next gate、permission境界変更 |
| [`plan/`](plan/) | Human review対象のslice別Implementation Plan artifactを保持し、Contractと実装開始gateの間をroutingする | Plan保持group・`CURRENT CANONICAL ROUTING`。各Planのdraft／adopted statusはartifact identityとContracts index／Roadmapを確認し、保存だけではimplementation permissionを生成しない | Tech Lead（技術計画）／Human（採用） | PKA | Contracts index、Roadmap、03／05／06のscope・DoD・QAと重なる | Planはtracked正本、ADRまたはadopted Contractを上書きせず、Plan adoptionとimplementation startを分離する | Contracts index、Roadmap、03／05／06 | AGENTS／CLAUDE、各Implementation Plan | Plan追加・採用・置換、identity、status、next gateまたはpermission境界変更 |
| [`operations/WTV-N7-EVENT-CREATION-ABUSE-PROTECTION-CLASS-M-OPERATION-PACKET-v0.1-draft.md`](operations/WTV-N7-EVENT-CREATION-ABUSE-PROTECTION-CLASS-M-OPERATION-PACKET-v0.1-draft.md) | Packet ID `WTV-N7-EVENT-CREATION-ABUSE-PROTECTION-CLASS-M-OPERATION-PACKET` v0.1-draftのexact external-operation packetのtarget、bound、STOP、evidence、Human gateを保持する | `HUMAN ADOPTED / CURRENT N7 CLASS M OPERATION PACKET AUTHORITY`。Independent Review `PASS`。Packet本文はimmutable reviewed snapshot | DevOps（operation semantics）／PKA（identity・lifecycle） | PKA | Contract／Plan／RoadmapのN7 lifecycleと重なる | Contract／Planを置換せず、packet adoptionからcredential、Class R、Class M execution、Vercel、Firewall、Hosted QA、Production permissionを生成しない | N7 Contract／Plan、Contracts index、Roadmap | Human adoption `N7_CLASS_M_PACKET_HUMAN_ADOPTION`／next gate `N7_CREDENTIAL_AND_CLASS_R_LIVE_PREFLIGHT_AUTHORIZATION` | packet作成・review・adoption・operation lifecycle変更 |
...
| [`Slice 5 Mission`](reports/pka-slice-5-supabase-governance-mission-and-dod-2026-07-20.md)／[`A1 publication record`](reports/pka-slice-5a-platform-metadata-inventory-2026-07-22.md) | abandonedされたSlice 5 GoalとA1完了事実、PR #17〜#19の履歴確認 | Missionは`CLOSED / GOAL ABANDONED / NOT IMPLEMENTED / HISTORICAL`、A1は`IMPLEMENTED / ACCEPTED / HISTORICAL PUBLICATION RECORD` | Human／Tech Lead／DevOps／PKA（各記録のdomain） | PKA | Roadmap、reports副索引と重なる | current authority、implementation input、次アクションまたは実行authorizationとして使用しない。現行Supabase運用はADR-0008・05・06・Skillを優先 | Roadmap、reports副索引、PR #17〜#19 | 履歴確認のみ | Humanによるlifecycle変更 |
...
| [`coordinate-claude-codex-worktree`](../.agents/skills/coordinate-claude-codex-worktree/SKILL.md) | agent／sliceごとの専用worktree標準、shared例外時のownership・lock・handoff | repository Skill・`CURRENT OPERATIONAL` | PKA（process）／Tech Lead（技術妥当性） | PKA | AGENTS／CLAUDEの原則、06 §1.1、closeout Skillと境界が重なる | publicationは06 §1.1、task-owned post-merge closeoutはcloseout Skillを優先 | 06 §1.1、closeout Skill | AGENTS／CLAUDE、agent Skill discovery | worktree標準、ownership、lock診断、handoff、routing変更 |
| [`check-execution-contract`](../.agents/skills/check-execution-contract/SKILL.md) | Execution Contract作成・受領・外部promptの最小品質検査 | repository Skill・`CURRENT OPERATIONAL` | Tech Lead（契約品質）／Human（採用判断） | PKA | AGENTS／CLAUDE、05、06、draft SkillのContract境界と重なる | canonical authorityを上書きせず、採用・実装permissionを生成しない | AGENTS／CLAUDE、05、06、draft Skill | agent Skill discovery | 品質原則、prompt分類、検査境界変更 |
| [`draft-execution-contract`](../.agents/skills/draft-execution-contract/SKILL.md) | Human明示発動によるExecution Contractのchat／Markdown draft | repository Skill・`CURRENT OPERATIONAL` | Human（方針・重要gate）／Tech Lead（契約・技術的意味） | PKA | AGENTS／CLAUDE、05、06のprompt・Human gateと重なる | 暗黙発動せず、契約生成とplan・実装を分離し、06のpublication／review権限を上書きしない | 05 §9.3、06 §1.0〜1.1 | AGENTS／CLAUDE、agent Skill discovery | 必須項目、出力形式、承認gate、review handoff変更 |
