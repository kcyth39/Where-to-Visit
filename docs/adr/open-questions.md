...
| B2  | 「確定」の扱い方     | 決定済（[ADR-0003](0003-evaluation-and-decision-logic.md)）— 確定行為は扱わず確定候補を自動ハイライトのみ                 |
| B3  | ○数が同数の場合の表示  | 決定済（[ADR-0003](0003-evaluation-and-decision-logic.md)）— 並列表示                                   |
| B4  | 確定候補の判定条件    | 決定済（[ADR-0003](0003-evaluation-and-decision-logic.md)）— ×が1つでもあれば除外、○1つ以上が対象、×候補はイシュー化を視覚明示（「理由確認中」テキスト不使用・非ブラックアウト）     |
...
| B8  | デフォルトと未評価の扱い | 決定済（[ADR-0003](0003-evaluation-and-decision-logic.md)）— デフォルト−、未評価と−は非区別、未参加者は評価表に非在、−は中間スコアなし |
...
| S2 | 表示名の入力タイミング | 決定済（[ADR-0004](0004-permission-model.md)）— 参加時に入力 |
...
| ID | 論点 | 状態 |
|---|---|---|
| C1 | イベントの保存期間 | 決定済（[03_requirements 非機能](../03_requirements.md#3-非機能要件)）— 無期限保存。ADR化しない |
| C2 | 候補数・参加者数の上限 | 決定済（[03_requirements 非機能](../03_requirements.md#3-非機能要件)）— 技術上限のみ（明示上限なし）。ADR化しない |
| C3 | 検索エンジンからの可視性 | 決定済（[03_requirements 非機能](../03_requirements.md#3-非機能要件)）— 非インデックス（URLはランダム長トークンで秘匿）。ADR化しない |
| C4 | イベント削除機能 | 決定済（[03_requirements 非機能](../03_requirements.md#3-非機能要件)）— 削除機能なし。ADR化しない |
...
> **全項目決着済み（2026-07-07）。** 新たな論点が出たら本リストに追記する。
