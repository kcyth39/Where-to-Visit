...
- **実装状態:** Supabase CLI 2.109.1とlocal stack設定は導入済み。localhost bind限定、接続先切り替え、local / remote E2E分離、advisor訂正は未実装
...
- 増分適用は`npx supabase migration list --local`と`npx supabase migration up --local`で確認する。
- local postflightでschema、RLS、policy、GRANT、function、trigger、FK、index、負系、不変条件、advisorを確認する。
- remote適用前の最終ゲートで、localデータ破棄を確認してから`npx supabase db reset --local --no-seed`を実行し、空DBから全migrationを再現する。
...
9. commit、push、Vercel確認、E2E cleanupをそれぞれ別承認にする。

## 失敗時

- localhost以外へのbind、target不一致、既存migration改変、local migration・postflight・advisor・E2E失敗ではremoteへ進まない。
- local DBは保持不要を確認したうえで`npx supabase db reset --local --no-seed`により再構築できる。
...
- `package.json`、起動wrapper、Playwright設定、tracked target allowlist、非追跡profileを実装する必要がある。
...
- localhost bind限定とtarget検証が完成するまで、現行の汎用`npm run test:e2e`をlocal E2E成功の証拠として扱わない。
