...
- ダッシュボード上部（Event情報の下、従来カード群の上）へ読み取り専用サマリー表を置く。1候補1行のsemantic tableで候補名（実リンク）・リンク（外部URL・別タブ）・総合評価`⭕️ / ➖ / ❌`・❤️ / 🌀を表示し、行余白クリックで候補編集へ進む。従来カードはその下に候補順で維持する。B-2詳細は[サマリー表・戻り導線 要件](reports/dashboard-summary-and-back-nav-requirements-2026-07-15.md)を正とする。

候補編集画面:

- Candidate情報と集約表示に加え、判断基準と全回答者行を表示する。
- ヘッダーの戻り導線「一覧に戻る」でダッシュボード（`/e/[shareToken]`）へ戻る。ダッシュボード表示中は同導線を非活性にする。B-1詳細は[サマリー表・戻り導線 要件](reports/dashboard-summary-and-back-nav-requirements-2026-07-15.md)を正とする。
...
| 候補一覧ダッシュボード | きめること・つたえておきたいこと / 上部に読み取り専用サマリー表（候補名・リンク・⭕️ ➖ ❌・❤️ / 🌀を1候補1行）/ その下にCandidateごとの総合評価・URL・追加時期・提案者・❤️ / 🌀集約カード。通常閲覧先 |
| 候補編集 | 1 Candidateの情報 / 判断基準 / 全回答者行 / 選択行の○・−・×・判断基準別❤️ / 🌀・コメント編集。ヘッダーの「一覧に戻る」でダッシュボードへ戻る |
