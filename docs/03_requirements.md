...
作成日: 2026-07-08 / 最終改訂: 2026-07-16 / フェーズ: Phase 1（要件定義）
...
> **B-3実装状態（local自動検証済み）:** トップとEventの5 view modeは、上段左=`Clarity Before Choice`（非リンク・System serif）、上段右=常設レイアウトスロット内のview mode別ナビ、下段中央=`きめのすけ`（`/`リンク）から成る共通ブランドヘッダーを用いる。375×812・320 CSS pxでも同じ上下関係と全文表示を維持する。200% resizeの手動確認とProduction確認は後続ゲートとする。詳細は[ブランドヘッダー刷新要件](reports/brand-header-refresh-requirements-2026-07-16.md)を正とする。
