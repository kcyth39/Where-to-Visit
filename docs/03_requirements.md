...
| ID | 受け入れ条件 |
|---|---|
| AC-2.1 候補追加 | タイトルまたはURLの少なくとも一方でCandidateを追加できる。フォーム見出しは「候補の追加」、入力ラベルは「候補名」とし、候補名inputにplaceholderとお名前欄を置かない |
...
- Candidate名から候補編集画面へ進む。追加時期・提案者、判断基準編集、回答者別詳細、コメント入力は候補編集画面へ集約する。
- ダッシュボード上部では、選択中回答者と控えめな変更buttonを最初に示し、その直下へ操作可能なサマリー表を置く。行全体は遷移させず、総合評価は表内で直接選択し、❤️ / 🌀は判断基準選択UIを開いて基準ごとに付け外す。同内容の候補タイルは置かない。候補追加はデスクトップでも候補名・リンク・追加buttonを1列に積む。B-2詳細は[サマリー表・戻り導線 要件](reports/dashboard-summary-and-back-nav-requirements-2026-07-15.md)を正とする。

候補編集画面:

- Candidate情報の直下で、サマリーと同じ○ / − / ×と判断基準別❤️ / 🌀を操作し、選択中回答者のコメントを明示保存できる。
...
- 「みんなの判断」の下に「候補情報編集」「判断基準編集」「判断者編集」を置く。候補削除は候補情報編集、判断基準の追加・label編集・2段階削除は判断基準編集、選択中回答者の名前変更・削除は判断者編集から行う。
