...
| AC-1.2 URL発行 | **Given** Event作成成功 **When** オーナー初期セットアップのステップ3を表示 **Then** 推測困難な共有URLとあなた専用URLを表示し、検索エンジンへ非インデックスとする |
| AC-1.3 未ログイン閲覧 | **Given** 共有URL保持者 **When** Eventを開く **Then** ログイン・登録なしでお題・メモを確認できる。回答者未選択なら名前選択、選択後は候補一覧ダッシュボードを表示する |
...
| AC-1.8 オーナー初期セットアップ | **Given** Event作成成功 **When** オーナー画面へ進む **Then** お題・メモの下に「お名前を入れる → 候補を挙げる → URLを送る」の3ステップを説明文付きで表示する |
| AC-1.9 owner再訪 | **Given** 別ブラウザまたは後日のowner URLアクセス **When** owner token検証に成功 **Then** 回答者未選択でも3ステップを再表示せず候補一覧ダッシュボードを表示し、お題・メモを編集できる。個人名義操作時だけ名前選択を求める |
...
- 共有URLの通常閲覧先とし、Eventのお題・メモと全Candidateを表示する。
...
お題作成・共有 / オーナー初期セットアップ / オーナー編集URL / 回答者セレクター / 候補一覧ダッシュボード / 候補編集 / ○・−・× / 最終候補3状態 / Criterion別❤️・🌀 / 1回答者1コメント / URLコピー / noindex / モバイル・デスクトップ対応 / 無期限保存。
...
| 画面 | 内容 |
|---|---|
| お題作成 | お題・メモ → 作成。属性・お名前・候補一覧リンク・イベント一覧は置かない |
| オーナー初期セットアップ | お題・メモ / 3ステップ説明 / お名前 / Candidate追加 / 共有URL＋あなた専用URL |
| ゲスト名前選択 | あなたのお名前 / 既存名選択 / 直下の直接入力。確定後は候補一覧へ |
| 候補一覧ダッシュボード | お題・メモ / Candidateごとの総合評価・URL・追加時期・提案者・❤️ / 🌀集約。通常閲覧先 |
