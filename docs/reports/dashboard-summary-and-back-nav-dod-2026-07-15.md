...
> 本書はフェーズB-1（戻り導線改善）＋ B-2（読み取り専用サマリー表）を同一リリースで完了と判断するための基準である。チェックが1つでも未達の場合は完了扱いにしない。**本スライスはUI専用であり、DB schema・migration・remote migration適用は発生しない**。ただし受け入れ検証はlocal Supabase依存E2E（`test:e2e:local`）であるため、`operate-supabase-live-db` Skillのlocal profile・target・localhost bind・E2E・cleanup・commit/pushゲートは適用される（§6・§7）。意図せずDB/データモデルへ手が入っていないことを逆に確認する。
...
- [x] 決定4点（一覧に戻る / 非活性=非リンク＋`aria-current="page"` / 375px2段grid / カードと同じcustom properties）が要件・DoD・QA・準備レポートの全箇所で一意に記述され、三択・「行または候補名」・「disabled」・「後で調整」等の緩い表現が残っていない（2026-07-15 同期で解消）
- [x] `ui-copy-decisions.md` の2026-07-15項目が承認済み（実装未着手）で、3点セット・`DESIGN.md`・`docs/reports/README.md`・`docs/03_requirements.md` とauthorityが一致している（2026-07-15 authority同期で解消）
...
- [ ] `dashboard` では戻り導線をリンクとして描画せず、同じ位置に「一覧に戻る」の非リンク要素（`event-nav-link is-disabled` 相当）を表示し、`aria-current="page"` を付与、クリック・Enter・Spaceで遷移しない（`<a disabled>` は使わない）
- [ ] `guest-selection` / `owner-setup` / `loading` は現行の文言・動作を変更していない（意図的維持）
- [ ] ヘッダー以外に新しい戻るボタン・パンくず等のナビ要素を追加していない
- [ ] active時は遷移先が分かるaccessible name、非活性時は `aria-current="page"` で現在地が分かる
...
- [ ] `DashboardSummaryTable` section が `EventHeading` の直後、既存 `section.dashboard-section` の前に挿入されている
- [ ] 既存 `dashboard-identity-bar` / `candidate-dashboard-grid` / `CandidateAddForm` / `ShareLinks` の構造と順序が変わっていない
- [ ] サマリー表とカード群が同じ `state.candidates` の並び順（`created_at ASC, id ASC`）で表示される
- [ ] desktopで semantic `<table>` を用い、列見出し「候補名 / リンク / ⭕️ ➖ ❌ / ❤️ / 🌀」と `<caption className="sr-only">候補のまとめ</caption>` がある（CSS gridの見せかけ表でない）
...
- [ ] 総合評価列に `⭕️`（positiveCount）/ `➖`（neutralCount）/ `❌`（vetoCount）が表示される
- [ ] `➖` が能動neutralのVote行数だけを数え、unratedを含めない
- [ ] ❤️列が `heartCount`、🌀列が `concernCount` を表示する
- [ ] サマリー表に追加時期・提案者・コメント列がない
- [ ] `decisionState` を**カードと同じCSS custom properties**で反映し、`clear/discussion/fallback` はsoft背景を行全体・前景色を先頭セル左境界（5px相当）へ、`none` は通常背景と `--line` に適用する
- [ ] 可視の状態説明ラベルを追加していない

### 操作

- [ ] 行内の非interactive領域（余白）クリック／タップで対象Candidateの候補編集画面へ遷移する
- [ ] 行コンテナに `role="link"` と `tabIndex=0` を重ねず、二重フォーカスを作らない。キーボード正規ナビは候補名の実リンク
- [ ] 行クリックhandlerが `a`/`button` 等interactive descendant発火のイベントを無視する
- [ ] URLリンクのクリックは別タブで外部URLを開き、行クリックの候補編集遷移を発火させない（伝播分離）
- [ ] サマリー表に○ / − / ×・❤️・🌀・コメントの編集controlがなく、個人名義操作が発生しない
- [ ] `DashboardSummaryTable` のpropsが `candidates: CandidateSummary[]` と `shareToken: string` の2つで、表示データは `CandidateSummary` 以外から再集計せず、`shareToken` はhref生成のみに使う
- [ ] `event-state.ts` / `event-types.ts` / actions / migration を変更していない
- [ ] 従来カード（`DashboardCandidateControls` 含む）と回答者名義controlの内容・操作・並び順が変わっていない

### 同期

- [ ] mutation成功後の完全状態再取得で、サマリー表とカードが同じ最新状態を反映する
...
- [ ] 375px で1候補を2段grid（1段目=候補名全幅＋URL全幅、2段目=⭕️➖❌・❤️・🌀を3領域横並び）にする。DOMは全幅で同一のsemantic table（`table/thead/tbody/tr/th/td`）を維持し、CSSの見せ方だけを2段化する（div一覧へ差し替えない）
- [ ] 375px でページ全体の横スクロールが発生せず、table wrapperだけの横スクロールへ逃がしていない。長いURLでも横溢れしない
- [ ] 375px で行余白クリック・候補名リンク・URLリンクのタップ対象が重ならず、誤タップしにくい
- [ ] 「行余白クリックで候補編集・候補名リンクはキーボード正規ナビ・URLは別タブ」の操作モデルがモバイル・デスクトップで一致する
- [ ] 戻り導線・候補名リンク・URLリンクに適切なaccessible nameがある
...
- [ ] Vercel反映後に独自ドメイン（kimenosuke.com）で主要導線（ダッシュボード表示 / サマリー行余白→候補編集 / 候補編集→一覧に戻る / URL別タブ）と 375×812・1366×768 のVisual QAを最小smokeとして実施する
