...
| DS-2.7 ❤️列 | **Given** サマリー行 **When** ❤️集計buttonを表示・操作 **Then** 枠のないカラー表示で候補全体の❤️合計を示し、判断基準選択dialogを開いて基準ごとに付け外せる。サマリー上では選択中回答者の選択状態を示さない |
| DS-2.8 🌀列 | **Given** サマリー行 **When** 🌀集計buttonを表示・操作 **Then** 枠のないカラー表示で候補全体の🌀合計を示し、判断基準選択dialogを開いて基準ごとに付け外せる。サマリー上では選択中回答者の選択状態を示さない |
...
- サマリー表の列は「候補名 / リンク / ⭕️ ➖ ❌ / ❤️ / 🌀」を最小構成とする。追加時期・提案者・コメントは候補編集画面に委ねる。❤️ / 🌀のdialogはCandidate名だけを見出しとし、「判断基準ごとの❤️・🌀」という説明文を置かない。
