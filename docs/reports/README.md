...
| PKA改善program実装準備 | Slice 3・4の共通遂行原則／Execution Contract要件と、Slice 5のSupabaseミッション | `APPROVED / NOT IMPLEMENTED`。個別sliceのExecution Contract／実行authorizationではない | Human／Tech Lead／PKA／DevOps（各domain） | PKA | 現行正本・Skill・Roadmapと一部重なる | 実装時は最新mainの正本と外部状態を再確認し、本groupだけで現行ruleを変更しない | docs入口、00、05、06、ADR-0008、Skill、Roadmap | 本索引、後続sliceのExecution Contract | 個別slice承認、要件・DoD・status・未決事項変更 |
...
- Slice 3・4: [`pka-slices-3-4-requirements-and-dod-2026-07-21.md`](pka-slices-3-4-requirements-and-dod-2026-07-21.md)
