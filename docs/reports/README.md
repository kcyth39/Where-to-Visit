...
| PKA改善program承認済み将来変更入力 | Slice 3・4の共通遂行原則／Execution Contract要件と、Slice 5のSupabase Mission／Process | `APPROVED / MIXED IMPLEMENTATION STATUS`。Slice 3・4は実装・受入・task-local closeout済み。Slice 5 A1は受入・merge済みで、PG-01 Minimal Process AmendmentはProcess Contract PRのHuman mergeまで`PROPOSED / NOT EFFECTIVE` | Human／Tech Lead／PKA／DevOps（各domain） | PKA | 現行正本・Skill・Roadmapと一部重なる | 現行ruleは最新mainの正本・Skillを優先する。個別Execution ContractとHuman承認なしに実装・運用を変更しない | docs入口、00、05、06、ADR-0008、Skill、Roadmap | 本索引、後続sliceのExecution Contract | 個別slice承認、要件・DoD・status・未決事項変更 |
| PKA Slice 5調査publication記録 | Slice 5 A1の実施・非実施・review gateを追跡 | 調査publication記録・`IMPLEMENTED / ACCEPTED / NO OPERATIONAL METADATA / NOT AUTHORIZATION` | PKA（構造）／各domain owner（review） | PKA | Slice 5ミッション、Roadmapと一部重なる | external stateの具体値・判定を保存せず、現行運用はADR-0008・05・06・Skillを優先 | Slice 5ミッション、ADR-0008、05、06、Skill | Roadmap、後続Execution Contract | A1 review、後続段階の承認、publication状態変更 |
...
次の3文書は、元V2.5の対応部分に代わるExecution Contract作成時の参照先である。現在有効なruleは最新mainの正本・ADR・Skillを優先し、各文書のmergeだけでは実装・運用を変更しない。個別Sliceまたは導入段階ごとのExecution ContractとHuman承認を必要とする。

- Slice 3・4: [`pka-slices-3-4-requirements-and-dod-2026-07-21.md`](pka-slices-3-4-requirements-and-dod-2026-07-21.md)。Slice 3は[PR #14](https://github.com/kcyth39/Where-to-Visit/pull/14)、Slice 4は[PR #15](https://github.com/kcyth39/Where-to-Visit/pull/15)で実装・受入済み。両Sliceともremote終了・task-local closeout完了
- Slice 5: [`pka-slice-5-supabase-governance-mission-and-dod-2026-07-20.md`](pka-slice-5-supabase-governance-mission-and-dod-2026-07-20.md)
- Slice 5 Process: [`pka-slice-5-supabase-governance-development-process-2026-07-22.md`](pka-slice-5-supabase-governance-development-process-2026-07-22.md)。PG-01 Minimal Process AmendmentとProcess Issue Registerを定義する。Process Contract PRのHuman mergeまでは`PROPOSED / NOT EFFECTIVE`であり、実行authorizationを付与しない

### PKA Slice 5調査publication記録

- A1: [`pka-slice-5a-platform-metadata-inventory-2026-07-22.md`](pka-slice-5a-platform-metadata-inventory-2026-07-22.md)。調査実施とreview gateだけを追跡し、具体的なsecurity／credential metadataは公開しない
