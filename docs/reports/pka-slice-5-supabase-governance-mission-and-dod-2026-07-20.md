...
- §12.1 amendment lifecycle: **Process Contract PR merge前 = PROPOSED / NOT EFFECTIVE**、**Human merge後 = APPROVED / EFFECTIVE**
- 作成日: 2026-07-20 JST
- repository: `kcyth39/Where-to-Visit`
- PG-01 rebaseline: `main` `4bdf5701b4b5bb80c9636c8026f4421f52258cd9`（A1 actual merge commit）
...
- authorityは文書名やstatusではなく、内容が担う役割により次の順で解釈する。ADR、Skill、その他の正本も一律の層へ固定せず、その内容が担う役割により分類する。
...
- 下位層は上位層を具体化できるが、上位根拠のないGoal、DoD、成果物、証拠、承認、禁止を追加または緩和しない。Process／QAが上位DoDを超える場合は、追加の仕組みで補完せずProcess defectとして扱う。
- Gate／QAを開始する前に、閉じる上位DoD、直接必要性、既存evidenceの再利用可否、本書および`AGENTS.md`／`CLAUDE.md`との整合を確認する。QAが実platformで実行不能な場合はDoDを拡張せず、QA／Processを訂正する。
...
- Process Contractおよびrepository／公式資料だけを用いるplan draftは、§12.1.1の条件で作成できる。外部状態確認またはmutationを伴う各導入段階は、§12.1.2の該当条件、最新外部状態を確認したExecution Contract、Humanの開始承認を必要とする。
...
- Humanは2026-07-22に、各GateのDoDが次工程の前提条件となる実行可能な順序とPR境界を設計し、旧planとの不一致があれば実行可能性を優先して修正するよう指示した。この指示は§12.1改訂の変更理由であり、改訂案、Process Contract PR、各Process Gateの実行開始を自動承認するものではない。
...
1. current stateをread-onlyで確定し、Human確認済みdev projectであるtracked `remote`と、Production／Preview application、Production、Stagingのtarget関係、credential、plan、drift、backup、MCP、GitHub integrationを確認する。
...
| remote E2E | `test:e2e:remote`はanon keyとrequest tokenによる通常application writeを行いうる。tracked `remote`はADR-0008によりHuman確認済みdev projectである。未確認なのはProduction／Preview applicationのcurrent Supabase targetである | `package.json`、`tests/helpers.ts`、`docs/adr/0008-local-supabase-development-workflow.md` |
...
- 2026年のSupabase Data API変更では、Data API exposure設定、SQL `GRANT`／default privilege、RLS／policyを三つの独立layerとして明示・検証する必要性が高まっている。既存projectへの強制時期を含め、実装時にChangelogを再確認する。[Data API GRANT change](https://supabase.com/changelog/45329-breaking-change-tables-not-exposed-to-data-and-graphql-api-automatically)

### 2.6 未確認事項

- Production／Staging／Preview Supabase projectの実在、用途、Reference ID、region、plan、Postgres version。
- Production／Preview applicationのcurrent Supabase target。applicationがtracked `remote`のdev projectを参照していても、`remote`をProductionへ再分類せず、environment分離が成立していない状態として扱う。
...
|---|---|---|---|---|
| SR-01 | `local`／`remote`だけでProduction・Stagingのapplication接続先を表現できない。tracked `remote`自体はdev projectである | 環境増加より先にlocal-first安全基盤を整えた | application接続先誤認、remote E2Eの影響範囲誤認、Productionとdevの未分離 | 最初のread-only inventoryでProduction／Preview applicationのcurrent targetを確定し、dev接続時は分離問題として停止する |
...
| ID | 要件 |
|---|---|
| SUP-I01 | Production、Staging／Preview、Localのproject、用途、Reference ID、plan、data方針、application接続先を値非表示のread-only証拠で確定する。tracked `remote`はHuman確認済みdev projectとして保持し、applicationが同projectを参照する場合はProductionへの読み替えではなくenvironment分離問題として記録する |
...
| SUP-I08 | Data API exposure設定、SQL `GRANT`／default privilege、RLS／policyを別layerで検証し、新規・変更table／functionの公開意図、Staging／Production差、2026年のplatform変更を実装時のcurrent Changelogで再確認する |
...
- [ ] Supabase project／branch／Local環境、用途、Production／Preview applicationのcurrent接続先が一覧化され、tracked `remote`がHuman確認済みdev projectとして一意に扱われている。applicationが同projectへ接続する場合はenvironment分離問題として停止条件と是正候補が記録されている。
...
| SUP-F06 | `SECURITY DEFINER`、privileged role、RLS bypass、workflow／secret／network変更を特別審査対象にし、Data API exposure設定、SQL `GRANT`／default privilege、RLS／policyを別々にtestする |
...
- [ ] Data API exposure設定、SQL `GRANT`／default privilege、RLS／policyが別々にtestされ、特権code／workflow／secret変更が特別審査される。
...
|---|---|---|---|
| environment | repo targetは`local`／Human確認済みdev projectである`remote`のみ。Production／Preview applicationのcurrent targetは未確認 | Local／Staging／Productionを一意に識別し、devとProductionを分離 | 外部inventoryと設計が必要。`remote`のProduction再分類は禁止 |
...
| SD-02 | Production／Preview applicationのtarget分離 | tracked `remote`はHuman確認済みdev projectで確定済み。未確認application targetの扱いと、dev接続時の分離方針が誤target防止とremote E2Eの許容riskに直結する | application targetが分離済みと確認／target Unknownのため停止／dev project接続を検出してenvironment・profile分離を再設計 | 現行application設定、command、E2E、credential配置へ影響。`remote`自体の意味は変更しない |
...
この改訂は、必須条件を削除せず、**Process採用・plan作成の前提**と、**各観測／mutation Gateを初めて開始する前提**へ分ける。すべての条件を詳細task分解前へ循環的に置くのではなく、初めて必要となるGateのEntry DoDとして満たす。先行GateのPASSは、後続Gateまたは別authorizationの開始承認ではない。

> **AMENDMENT AUTHORITY**
>
> この§12.1改訂と詳細Process文書を同じProcess Contract PRでHumanが承認・mergeするまでは、従来の「全条件充足前に詳細task分解へ進まない」という境界を維持し、詳細Processは提案資料として停止する。

#### 12.1.1 Process採用・plan作成の必須条件

次をすべて満たした場合だけ、Process Contract PRをmergeし、GateごとのExecution Contract／plan draft作成へ進める。これらのPASSはDB、Docker、外部状態、GitHub／Supabase settings、credential、Git publicationの変更を許可しない。
...
- [ ] 実行対象repository、Process baseline、branch／worktree、owner、GitHub publication境界が一意で、他ownerのdirty変更と分離されている。
- [ ] SUP-GOV-REF-01の永続保管方針が承認されている、または受領pathを維持する期限・owner・再判断条件が記録されている。
- [ ] Tech Lead、DevOps、独立Reviewer、Human approverがProcess設計に割り当てられ、Tech Lead／PKAのcode実装禁止とReviewerのwrite制限を維持できる。
- [ ] current Supabase Changelog、公式Docs、対象CLI `--help`、repository visibility、GitHub plan制約をread-onlyで確認し、Process上の未確認事項と再確認Gateが記録されている。
- [ ] Process Contract PRのmerge／pushからProduction DB mutationへ至るGit-trigger経路を、Supabase Dashboard GitHub repository連携、repository workflow／webhook、Installed GitHub Apps、具体的GitHub／Supabase連携という有限管理surfaceでread-only分類する。具体的兆候のない管理外経路は`NotCompared`として不存在を推定せず、有限surface上の経路があれば現行正本下の別Execution ContractとHuman承認で分離する。PG-00C2のmerge直前postcheckは、この有限surfaceの`PATH_ABSENT / Confirmed`分類に影響する設定変更がないことのcurrentness確認だけを担う。Humanによる「既知のProduction DB変更が現在実行中または予約中ではない」というoperational attestation、merge開始からDB post-merge観測完了までのchange freeze、`EXPECTED_DB_EFFECT = NONE`、path分類に対応するpost-merge DB観測planとauthorizationは、PG-00Mのmerge準備・Human merge判断で固定する。`PATH_PRESENT`時だけ既存Production DB deployment metadataでschema／migration effectを観測する。`PATH_ABSENT`時は存在しないmetadataを要求せず、current finite-surface identity、actual merge commit、有限windowに基づきGit-triggered DB deployment observationを`NOT_APPLICABLE_BY_PATH_ABSENCE / Confirmed`として閉じ、schema全体またはDB全体の不変を主張しない。Vercel Production applicationは2026-07-23のHuman判断により現行Git-triggerを維持する。merge前はapproved PR Head SHA、current base SHA、通常merge方式、current Vercel project／Production branch／Git-trigger設定、および結果として生成されるmerge commitをsourceとする想定済み1 deploymentというexpected source ruleを固定し、future merge SHAを推定しない。merge後にactual merge commit SHAを固定し、そのSHA由来のdeploymentがexact 1件、READY／Current、期待alias／domain一致、想定外settings／alias delta 0であることを確認する。path分類に対応するDB結果とapplication `EXPECTED_APP_DEPLOYMENT / Confirmed`を別々に観測し、同一merge identityでrollupしている。有限surfaceのdrift、既知のDB変更、change freeze不成立、path分類に対応するpost-merge DB観測plan未承認、Head／base／merge方式／Git-trigger設定のdrift、applicationの0件／複数件／別source／failed／stale／想定外delta／Unknownは停止する。
- [ ] 本§12.1改訂、詳細Process文書、Roadmap、副索引が同じProcess Contract PRの同一exact Headにあり、独立review後にHumanがmergeする。

#### 12.1.2 各観測／mutation Gateの必須条件

| 必須条件 | 初めてPASSを必要とするGate | それ以前に許可できる範囲 |
|---|---|---|
| Supabase／GitHub外部状態を値非表示・read-onlyで確認するexact scopeとauthorization | A1および各AC0／AC3観測Gate | repository／公式資料のread-only確認、plan draft |
| repository、baseline、branch／worktree、owner、publication境界のcurrent再確認 | 各Execution Contract開始 | plan draft。drift時は実行不可 |
| 当該GateのTech Lead、DevOps、独立Reviewer、Human approverが割り当てられ、role制約を維持できる | 各Execution Contract採用・Gate開始 | role候補を示すplan draft。未割当のまま実行不可 |
| Production、Staging、tracked `remote` dev、applicationのtarget identityと、Reference ID、Database hostname、port、database、roleの分離 | A2-T1、D1a／D1bおよび各Hosted Gate | targetを使わないrepository設計。application targetがUnknownまたはdev接続ならHosted／Production Gateを開始しない |
| current Changelog、Docs、CLI `--help`、GitHub plan／visibility制約 | 各該当Gate開始直前 | 古いsnapshotをcurrent事実にしないplan draft |
| Production write credentialへLocal AI／PR jobが到達しない設計と、値非表示の検証方法 | B3a／B3b。credential／deploy Gateでは再確認 | credentialを作成・参照しないplan／Local設計 |
| 未解決High riskがない。残す場合はowner、risk、暫定対策、再開条件、Human確認を記録し、当該導入段階のProduction対象から明示的に除外する | 各mutation Gate。Production Gateでは再確認 | 対象外として明示したlow-risk plan／read-only確認 |
| 各導入段階の最初のmutationがlow-risk、可逆的、効果測定可能で、主指標1件とrollback1件を持つ。Production pilot candidateにも同じ条件を再適用する | 各導入段階の最初のmutation Gate、およびPilot Candidate Gate | plan／read-only確認。条件未充足のmutationは禁止 |
| 実在する別Human approver、required reviewerまたは承認代替案の実現可能性 | B1／PG-03Aで早期判定し、Environment／Production Gate直前に再確認 | approver／secretを使わないrepository設計 |
| pilot限定のHuman-triggered protected schema routeと現行SQL Editor fallbackの関係が、AGENTS／CLAUDE、`docs/README.md`、03〜06、ADR-0008、Supabase運用reference／Skillのexact 14-file正本群へ一貫して反映され、03の内容責任者と04の内容責任者を含むdomain review、Human採用、独立review、Human merge、post-merge両surface観測、main currentness確認を完了している | E2-G／PG-24S0〜PG-24S11。PG-25Aより前 | 正本変更案の作成・reviewだけ。current正本のSQL Editor経路を一般則／fallbackとして維持し、pilot限定例外から一般remote writeを許可せず、settings、credential、deploy、DB writeを行わない |
| pilot後のSQL Editor dispositionとprotected routeの通常／fallback／禁止が、同じexact 14-file正本群へsurface別に反映され、同じ内容責任者確認、domain review、Human採用、独立review、Human merge、post-merge両surface観測、main currentness確認を完了している | F1／PG-28〜PG-28L。PG-29およびCycle 2のProduction packageより前 | PG-28のHuman判断と正本変更案の作成・reviewだけ。未発効decisionをcurrent authorityとして使わず、settings、credential、deploy、DB writeを行わない |

条件が不成立なら、当該条件を初めて必要とするGateで有限停止する。未充足条件を安全と推定したり、後続GateのPASSから遡って補完したりしない。各導入段階のExecution Contractと実装開始は引き続き未承認であり、HumanがGateごとに判断する。

GateごとのHuman判断は維持する。ただし、各Gate、許可操作、Entry、STOP、失効条件を個別に明示し、authority、target、scope、許可操作、runtime principal、停止・rollback境界、bundle-level input identity／lineage、riskがすべて同一である有限な作業だけは、一つのHuman approval packet／一回の応答へまとめられる。bundleはGateごとの判断を省略せず、対象Gateを明示的に内包するものであり、bundle外のGateまたはmutationへの暗黙の後続authorizationを作らない。Entry未充足、identity／lineage drift、または失効条件成立時は、同じbundle内でも後続Gateを開始しない。
