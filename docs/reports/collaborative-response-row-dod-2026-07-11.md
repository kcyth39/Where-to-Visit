...
- [ ] ADR-0005の旧current participant / guest_token依存と候補単位常設🌀へSUPERSEDED境界がある
...
- [ ] 候補編集画面に既存の`Candidate.created_at`を基準とする追加時刻が表示される
- [ ] Candidateのタイトル・URL・提案者・回答編集で`created_at`が変わらない
- [ ] Reaction / Concern / Commentは選択対象の同一event Participant名義で共同編集できる
- [ ] Concernに`criterion_id NOT NULL`と`UNIQUE(candidate_id, participant_id, criterion_id)`があり、Candidate / Participant / Criterionの同一eventをDBで保証する
- [ ] 同じCandidate×Participant×CriterionにReactionとConcernを両方作成できる
...
- [ ] トップにEvent内の候補一覧リンクとイベント一覧を実装していない
- [ ] オーナー初期セットアップに「お名前を入れる / 候補の追加」のタイトルと確定説明文が表示される
- [ ] オーナーは名前確定後も同じ画面でCandidate追加へ続き、「さあ、きめよう！」後に初回共有ステップを表示する
- [ ] 初回共有ステップで共有URLをコピーでき、「わたしの意見を入力」で同じタブの候補一覧へ進む
- [ ] owner URLによる再訪は回答者未選択でも候補一覧を表示し、初期セットアップを再表示しない
- [ ] 初期セットアップmodeをDBへ永続化せず、reload・再訪では候補一覧へ進む
- [ ] 未選択の再訪オーナーはお題・メモを編集でき、個人名義操作時だけ名前選択へ進む
- [ ] 未選択ゲストには名前選択だけを表示し、既存名選択肢の直下に「直接入力」がある
- [ ] 既存名を選ぶと直接入力へ名前が反映され、Participantを増やさず候補一覧へ進む
- [ ] 新名確定後に候補一覧へ進み、有効なlocalStorage選択での再訪は名前選択を省略する
...
- [ ] 候補追加フォームにお名前欄がない
- [ ] 候補追加フォームの見出しが「候補の追加」、入力ラベルが「候補名」で、候補名placeholderがない
- [ ] 回答者未選択かつ名前draftなしでもCandidate / CriterionをParticipant生成なしで追加・編集・削除できる
- [ ] 候補一覧にEventのお題・メモと全Candidateの集約が表示される
- [ ] 候補一覧はCandidate名、URL、`⭕️ / ➖ / ❌`、❤️ / 🌀合計を1候補1行のサマリーへ集約し、重複する候補タイルを表示しない
- [ ] 候補一覧に❤️／🌀反応項目編集、回答者別編集control、コメント入力を展開していない
- [ ] Candidate名から対象の候補編集画面へ進める
- [ ] 候補編集上部にサマリーと同じ○ / − / ×、❤️ / 🌀controlとコメント入力が表示される
- [ ] 回答者行に、名前、評価、判断基準別❤️ / 🌀、コメントが表示される
- [ ] 全回答者行は現在値の閲覧専用で、個人名義の編集controlを表示しない
- [ ] 回答者行clickでは選択中回答者も回答値も変更しない
- [ ] 選択中回答者の個人名義controlはCandidate情報上部だけに表示される
- [ ] 選択中回答者を「〇〇として判断中」と表示する
- [ ] 専用の変更controlから回答者を選ぶと、画面全体の選択中回答者が切り替わる
...
- [ ] 判断基準は「＋ 判断基準」と各基準の「…」から追加・label編集・2段階削除できる
- [ ] 同名・変更・削除確認は現在の対象について1画面だけを表示し、確認画面をタイル状に並べない
...
- [ ] 候補編集ヘッダに、60分未満、1〜23時間、1日以上の作成時刻が仕様どおり表示される
...
- [ ] 可視の状態説明ラベルがなく、支援技術向け状態名と`⭕️ / ➖ / ❌`の実数でsemantic colorを補完している
...
- [ ] Criterionごとの🌀数と付与者も表示される
- [ ] 同じ回答者が同じCriterionへ❤️と🌀を両方付けられる
- [ ] 候補全体の🌀数がCriterion別Concern行の単純合計である
- [ ] Candidate単位の常設単一🌀が存在しない
- [ ] ❤️・🌀・コメントが3色判定へ混入しない
...
- [ ] トップ、オーナー初期セットアップ、ゲスト名前選択、候補一覧、候補編集、同名確認、名前変更、回答者削除、3色状態を確認できる
- [ ] 375×812でページ全体の横overflowがない
- [ ] 1366×768で候補一覧の候補間、候補編集の回答者間を比較しやすい
- [ ] デスクトップとモバイルで、上部は自分の操作・回答者一覧はread-onlyという操作モデルが一致する
- [ ] モバイルで候補一覧カードと候補編集内の情報が重ならず、操作対象が明確である
- [ ] モバイルを含む全幅で回答者コメントを省略せず全文表示する
- [ ] ボタン・入力・ダイアログに適切なaccessible nameがある
- [ ] 可視の説明ラベルを追加せず、支援技術向け状態名と評価実数で色を補完する
- [ ] 選択中回答者と編集対象が誤認されない
- [ ] 375×812 / 1366×768の実画面を人間確認し、色・評価chip・Candidate追加時刻表示を調整している
