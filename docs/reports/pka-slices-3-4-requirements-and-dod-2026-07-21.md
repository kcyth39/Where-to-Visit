...
| Slice 4 | APPROVED | H-03補完と実装開始をHuman承認済み。固定PR URLはDraft PR作成後に同一branchで追記し、current publication状態はGitHubを正とする。Human mergeまでは現行ruleへ未発効 |

本書は要件、Human判断、lifecycleだけを保持する。Slice 3の実装・検証証拠はPR #14本文、Slice 4の実装・検証証拠は固定PR本文を参照し、本書へ複製しない。
...
次はHuman（おしげさん）が本対話で確定し、Slice 4の実装開始承認で採用した要件補完である。実装証拠は固定PR本文へ保存し、本節へ複製しない。
