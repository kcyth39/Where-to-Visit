...
4. **B-2 サマリー行の操作** → 行全体は遷移させず、候補名リンク・外部URL・表内評価controlを役割分離する。
...
### B-2 操作可能サマリー表（要件ID `DS-2.x`／`RS-3.x`）

- ダッシュボード上部（きめること・つたえておきたいことの下）で、選択中回答者と控えめな変更buttonの直下に新設。候補一覧と操作はサマリー表へ一本化し、重複する候補カードは置かない。
- 1候補1行で全Candidateを既存 `state.candidates` の並び順で表示。
- 列は「候補名／リンク／⭕️ ➖ ❌／❤️／🌀」の最小構成。追加時期・提案者・コメントは出さない。
- 行余白は非操作。候補名は候補編集への実リンク（キーボード正規ナビ）、URLリンクは別タブで開く。
- 総合評価は表内で直接選択し、❤️ / 🌀は判断基準選択dialogから基準ごとに付け外す。コメント編集controlは置かない。
- `decisionState` のsemantic colorと状態名を既存仕様と同じ意味で反映。
- mutation成功後の完全状態再取得でサマリー表へ最新状態を反映（独自fetch・pollingなし）。
...
| BN-1.5 | ダッシュボード上での扱い | **実機確認を踏まえ、戻り導線を表示しない** |
| RS-3.2 | モバイル表示 | **375pxは指定の2段grid**（DOMはsemantic table維持、見せ方だけ2段化）。ページ／wrapper横スクロールなし |
| DS-2.12 | サマリー行への色反映 | **既存のCSS custom properties**（soft背景を行全体・前景色を先頭セル左境界、`none`は通常面＋`--line`） |

反映先:

- 要件定義書 §4 の該当ID本文へ「決定・2026-07-15」を明記し、§8を「未決事項」→「決定事項（解消済み）」へ書き換えた。
- `ui-copy-decisions.md` に改訂履歴（2026-07-15）を追加し、イベント詳細画面の確定コピー表へ「ヘッダー戻りリンク＝一覧に戻る／ダッシュボードでは非表示」を反映した。
...
- 戻りリンク文言「一覧に戻る」・ダッシュボードでの非表示・サマリー行の色統一が意図どおりか。
- サマリー表の列（候補名／リンク／⭕️➖❌／❤️／🌀）と操作（行余白は非操作、候補名→候補編集、URL→別タブ、評価→同一画面内mutation）で過不足がないか。
