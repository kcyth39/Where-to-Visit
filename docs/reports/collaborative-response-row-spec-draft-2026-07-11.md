...
Next.js App Router
  ...
  ├─ Client UI: 回答者選択、候補サマリー、候補編集、ダイアログ、draft
...
- owner URL検証成功またはowner Cookieによる後日の再訪は、selected participantがなくても候補一覧ダッシュボードを表示する。初期セットアップは再表示しない。
...
- 候補編集ヘッダに`created_at`由来の「1時間以内に追加 / N時間前に追加 / N日前に追加」を表示する。
...
デスクトップ・モバイルとも、きめること・つたえておきたいことの直後に2つの入力セクションを置く。入力完了後は同じ画面を初回共有ステップへ切り替える。
...
2. 候補の追加
候補名 [________________]
リンク [________________]
[追加]
...
- 名前確定後も同じ画面に残り、Candidate追加へ続く。
- 「さあ、きめよう！」後は共有URLを中央に表示し、「わたしの意見を入力」で同じタブの候補一覧へ進む。
- 「直す」はお題・メモとの結び付きを示しつつ小さくする。
- 候補名inputにplaceholderは置かない。
- モバイルでも各入力とbuttonを1列に積み、説明文を省略しない。
...
デスクトップではEventのきめること・つたえておきたいことと複数Candidateを十分な幅で表示する。Candidate一覧はサマリー表へ一本化し、モバイルも同じ情報順序を維持する。
...
┌──────────────────────────────────────────────────────┐
│ 川沿いのカフェ  https://example.com/cafe  [⭕️5][➖1][❌0] ❤️8 🌀2 │
│ 公園でピクニック URLなし                  [⭕️3][➖2][❌1] ❤️4 🌀3 │
└──────────────────────────────────────────────────────┘

候補名 [__________________________________________]
リンク [__________________________________________]
[追加]
```

- Candidate名を候補編集への導線とし、同じCandidateを繰り返す候補タイルは置かない。
- `⭕️ / ➖ / ❌`は別々のchipとし、`➖`が背景へ埋もれないcontrastを確認する。数値を式のように連結しない。`➖`はneutralだけを数え、unratedを含めない。
- ○ / − / ×を主要情報、❤️ / 🌀を小さな補助情報として視覚階層を分ける。❤️ / 🌀はCandidate名だけを見出しとする判断基準選択dialogを開く。
- Eventのお題とメモはスクロールで消えてよい。
- `clear / discussion / fallback / none`の可視説明ラベルは置かない。サマリー行の控えめなsemantic colorと支援技術向け状態名を使う。
- ❤️／🌀反応項目編集、回答者別control、コメント入力を候補一覧へ展開しない。
...
田中として判断中
[⭕️ 5] [➖ 1] [❌ 0]  [❤️ 8] [🌀 2]
コメント [____________________________] [保存]

お名前  総合評価   判断基準への反応              コメント
田中    ○           興味❤️ 価格❤️🌀                 とてもよさそう
鈴木    ○           興味❤️                          よさそう
佐藤    ×           価格🌀                          場所が気になる
...
- Candidate情報と同じ候補タイル内にサマリーと同じ○ / − / ×、❤️ / 🌀を置き、コメント入力をその直下へ置く。
- 候補内容の編集だけを＋／−付き開閉UIからインラインpanelで開く。反応項目の追加・名称変更・2段階削除は「❤️／🌀反応項目の編集」modalへ収め、追加buttonは既存一覧の下に置く。modal導線2件はデスクトップで同一行・文言改行なし、モバイルで横幅不足時だけボタン単位・文言とも折り返せる。選択中回答者の名前変更・削除は「判断者名の変更／削除」modalへ収め、現在名inputと変更・キャンセル・右端の削除を1画面に置く。削除確認中は編集UIを隠し、各確認段階を「消す／キャンセル」の2択にする。
- ❤️ / 🌀は判断基準ごとに独立して付けられ、同じ回答者が同じ基準へ両方付けられる。Candidate単位の常設🌀は置かない。
- 全回答者行はread-onlyとし、行clickではselected participantを切り替えない。
- モバイルでは回答者行を縦に積み、コメントを省略せず全文表示する。
...
> **実装時訂正（2026-07-13）:** 以下の「実DB全削除」「Event 0件guard」を前提とするdelete-first方式は採用しなかった。通常Eventは保持し、DDL前guardで決定的に変換できないデータだけを停止させるデータ保持型migrationを実装・適用済みである。現行仕様は[データモデル §移行方針](../04_data-model.md)と[実装ステータスレポート §3.4](db-implementation-and-development-status-2026-07-13.md)を正とする。`[E2E]`データのcleanupはmigration前提ではなく、検証後の後処理として別ゲートで行う。

- 適用済みmigrationは編集しない。
- 新規migrationは`npx supabase migration new <name>`で作成する。
- 通常Event、Participant、Criterionと、決定的に変換できる関連行は既存IDを維持してin-place移行する。
- DDL開始前にParticipant名、重複、NULL、cross-event参照、Comment一意性、Concern backfillの決定性をguardし、変換不能ならtransaction全体を停止する。
- cleanupは`[E2E]`prefixの明示対象だけに限定し、人間確認・rollback検証・commit承認を伴う別SQLとする。
- remote適用方式はCLI導入後も人間によるSQL Editor全文実行を維持する。

### 11.6 Remote cleanup順序

> **SUPERSEDED（2026-07-13）:** 旧schema全Event cleanupは、データ保持型migrationへの変更により本筋migrationの前提ではなくなった。通常Eventは削除しない。

現行cleanupは検証後に残る`[E2E]`prefixの明示対象だけを、次の順で後処理する。

1. fresh discoveryで対象Event UUID、依存件数、schema／FK／trigger／invariantを記録する。
2. 固定UUIDと`[E2E]`prefixの二重条件を持つROLLBACK SQLを生成する。
3. ROLLBACK後にbaselineが完全復元したことをSELECT-only postcheckで確認する。
4. 別承認後、同じscope digestのCOMMIT SQLを生成して一度だけ実行する。
5. 固定UUID、残存`[E2E]`件数、全関連table、schema invariantをSELECT-only postcheckで確認する。
...
9. remote旧schemaの通常Eventを保持したまま、データ保持型migrationのpreflightで変換可能性とmanifestを確認する。
...
### U-2. 回答者一覧と個人操作の分離（2026-07-16再確定）

- 決定日: 2026-07-11
- 全回答者行はread-onlyとし、現在値とコメント全文を表示する。
- 回答者行には○/−/×・❤️・🌀・コメントの編集controlを表示しない。
- 行本体を選択してもselected participantを切り替えない。
- 個人操作はCandidate情報上部、回答者変更は専用controlへ集約する。
...
### U-5. 回答者一覧のコメント表示量（2026-07-16再確定）

- 決定日: 2026-07-11
- デスクトップ・モバイルともコメントを省略せず全文表示する。
- コメント編集textareaはCandidate情報直下へ置き、回答者一覧には置かない。
...
- オーナーは名前確定後も初期セットアップ画面に残り、共有リンク表示後に「わたしの意見を入力」で候補一覧へ進む。ゲストは名前確定後に候補一覧へ進む。有効なlocalStorage選択で再訪したゲストは候補一覧を直接表示する。
