...
- 判断基準dialog: Candidate名と判断基準行だけを表示し、「判断基準ごとの❤️・🌀」という説明見出しは置かない。
...
- コメント: 候補タイル直下へ選択中回答者の入力欄と保存buttonを置く。コメントは「みんなの判断」の行内では編集しない。
- みんなの判断: 全回答者の名前、総合評価、判断基準別❤️ / 🌀、コメント全文を読み取り専用で表示する。行clickによる回答者変更や行内編集controlを置かない。
- 詳細編集: 「みんなの判断」の下へ「候補情報編集」「判断基準編集」「判断者編集」の3メニューを置く。候補削除は候補情報編集、判断基準の追加・名称変更・削除は判断基準編集、選択中回答者の名前変更・削除は判断者編集へ収める。
