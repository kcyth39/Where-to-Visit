...
1. Read the repository's governing `AGENTS.md` and its canonical Git workflow.
...
3. Require evidence that the PR is `MERGED` and that its recorded Head SHA equals the local branch Head.
...
4. Verify the local Head equals the merged PR Head.
5. Verify that local Head is an ancestor of the latest fetched baseline with `git merge-base --is-ancestor <exact-local-head> <exact-baseline-ref>`.
...
- no tracked or untracked change;
...
- no submodule, lock, or repository-state ambiguity; and
- ignored files are fully classified.
...
2. Confirm the target worktree is no longer registered and the control location remains clean.
...
- tracked, untracked, unpublished-commit, ignored-file, residual-work, and ownership checks;
